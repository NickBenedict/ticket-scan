...
public class SellingHandler {
	...
	public static String getAllSessionCookie(String username, String password) {
		...
		try {
			URL url = new URL(Constants.MYACCOUNT_REST_SERVER + "/myaccountapi/myaccount/user/userGuid/session");
			...
			Log.e(Constants.TAG, "Error getting session cookie: " + Log.getStackTraceString(e));
			return "";
		}
	}
	...
	public static String getSessionCookie(String username, String password) {
		...
		try {
    		...
    		for(int i = 0; i < values.size(); i++) {
    			if(values.get(i).startsWith(Constants.SELLER_PRO_SESSION_COOKIE)) {
	...
	public static ListingResponse attachBarcodeToListing(String sellerGuid, String sessionCookie, String listingId, List<String> barcodes) {
		HttpURLConnection con = null;
		ListingResponse response = new ListingResponse();
		
		try {
			URL url = new URL(Constants.SELLER_PRO_SERVICE_ROOT + "/api/fulfillment/seller/" + sellerGuid + "/listing/" + listingId + "/barcodes");
			...
			for(int i = 0;i < barcodes.size();i++) {
				sb.append("<barcodeSeat><barcode>" + (String)barcodes.get(i) + "</barcode></barcodeSeat>");
			}
			...
			if(errorCodeList==null || errorCodeList.getLength()==0) {
				...
				response.setListingId(getTextContent((Element)doc.getElementsByTagName("listingId").item(0)));
			}
			else {
				response.setSuccessFlag(false);
				response.setErrorCode(getTextContent((Element)errorCodeList.item(0)));
				response.setErrorType(getTextContent((Element)doc.getElementsByTagName("errorType").item(0)));
			}
			return response;
 	...
 	public static ListingResponse addListing(String sellerId, String sessionCookie, String eventId, String quantity, String section, String row,
			String seats, String faceValue, String ticketPrice, String currency, String splits, String inHandDate,
			String fulfillmentType, List<String> disclosures) {
 		ListingResponse response = new ListingResponse();
		...
		try {
			...
			URL url = new URL(Constants.SELLER_PRO_SERVICE_ROOT + "/api/listings/seller/" + sellerId);
    		...
    		/*String cookie = "STUB_PERSISTENT=filler%7E%5E%7E1%7Cstub_uid%7E%5E%7E13877497%7E%5E%7E05%2F04%2F2012; Domain=.stubhub.com; Expires=Sun, 27-Apr-2042 06:24:02 GMT; Path=/; " + 
			...
			*/
			...
			sb.append("<pricePerTicket><amount>" + ticketPrice + "</amount><currency>" + currency + "</currency></pricePerTicket>");
			if(faceValue!=null) {
				sb.append("<ticketFaceValue><amount>" + ticketPrice + "</amount><currency>" + currency + "</currency></ticketFaceValue>");
			}
			if(splits!=null) {
			...
			sb.append("<deliveryOption>" + fulfillmentType + "</deliveryOption>");
			for(int i = 0; i < disclosures.size(); i++) {
				sb.append("<ticketTrait><name>" + disclosures.get(i) + "</name><type>Listing Disclosure</type></ticketTrait>");
			}
    		...
    		//String ss = "<listingRequest><eventId>"+ eventId + "</eventId><section>2</section><row>3</row><pricePerTicket><amount>9999.00</amount><currency>USD</currency></pricePerTicket><quantity>1</quantity><ticketInHandDate>" + inHandDate + "</ticketInHandDate><deliveryOption>" + fulfillmentType + "</deliveryOption></listingRequest>";
	        ...
	        */
			...
			Log.e(Constants.TAG, "Error adding listing: " + Log.getStackTraceString(e));
			return null;
		}
		finally {
			if(con != null) {
	...
	public static EventInfo getEventInfo(String eventId, String sessionCookie) {
		...
		try {
		    ...
		    /*  setting this because the SelerPro certificates are not recognized by Android.
		     * 
		    Android 2.X does not support Server Name Indication so if your server is relying on it for SSL handshaking, Android may not be getting the certificates you're expecting. 
		    Do you have certificate chain on the server installed, and is it ordered correctly? Most browsers handle out-of-order certificate chains but Android 2.X does not. 
		    
		    */
			
			URL url = new URL(Constants.SELLER_PRO_SERVICE_ROOT + "/api/events/" + eventId);
			Log.d("Koson","get eventInfo url is: " + Constants.SELLER_PRO_SERVICE_ROOT + "/api/events/" + eventId);
	        ...
	        Log.d("koson","message is " + sb.toString());
	        */
	        
			Document doc = db.parse(new InputSource(con.getInputStream()));
			doc.getDocumentElement().normalize();
			
			NodeList deliveryOptionList = doc.getElementsByTagName("fulfillmentMethod");
			Log.d("Koson","the length is " + deliveryOptionList.getLength());
			for(int i = 0; i < deliveryOptionList.getLength();i++) {
				Element name = (Element)((Element)deliveryOptionList.item(i)).getElementsByTagName("name").item(0);
				Element endDate = (Element)((Element)deliveryOptionList.item(i)).getElementsByTagName("endDate").item(0);
			...
			for(int i = 0; i < ticketTraitList.getLength();i++) {
				String name = ((Element)((Element)ticketTraitList.item(i)).getElementsByTagName("name").item(0)).getFirstChild().getNodeValue();
				String type = ((Element)((Element)ticketTraitList.item(i)).getElementsByTagName("type").item(0)).getFirstChild().getNodeValue();
			...
			Log.e(Constants.TAG, "Error getting event info " + Log.getStackTraceString(e));
			return eventInfo;
		}
		finally {
			if(con != null) {
